...
class CameraMan {
  ...
  let queue = DispatchQueue(label: "no.hyper.ImagePicker.Camera.SessionQueue", qos: DispatchQoS.background, attributes: [], autoreleaseFrequency: DispatchQueue.AutoreleaseFrequency.inherit, target: nil)
  //let queue = DispatchQueue(label: "no.hyper.ImagePicker.Camera.SessionQueue", qos: dispatch_queue_attr_make_with_qos_class(DispatchQueue.Attributes.serial, DispatchQoS.QoSClass.background, 0))
